...
try:
    ...
    from langdetect.lang_detect_exception import LangDetectException as _LANGDETECT_EXCEPTION
    _HAS_LANGDETECT = True
except ImportError:
    _langdetect = None
    _LANGDETECT_EXCEPTION = Exception
...
class LanguageDetector:
    ...
    def detect(self, text: str) -> str:
        ...
        if self.use_cache:
            if len(self._cache) >= self.cache_max_size:
                self._evict_half_cache()
            self._cache[text] = result

        return result

    def _evict_half_cache(self):
        """Evict the oldest half of the cache without copying all keys."""
        keys_to_evict = []
        for idx, key in enumerate(self._cache):
            if idx >= len(self._cache) // 2:
                break
    ...
    def detect_with_confidence(self, text: str) -> Tuple[str, float]:
        ...
        if (self._has_langdetect
            ...
            except _LANGDETECT_EXCEPTION as exc:
                self.logger.log_event(
                    "langdetect_error",
                    f"langdetect failed, falling back to heuristics: {exc}",
                    level="WARNING",
                )
    ...
    def _detect_single(self, text: str) -> str:
        ...
        if self._has_langdetect:
            ...
            except _LANGDETECT_EXCEPTION as e:
                self.logger.log_event(
                    "langdetect_error",
