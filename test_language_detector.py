...
class TestLanguageDetectorConfidence(unittest.TestCase):
    ...
    def test_detect_with_confidence_langdetect_exception_falls_back(self):
        """Specific langdetect failures should fall back to heuristics."""
        detector = LanguageDetector()
        ...
        with patch("language_detector._LANGDETECT_EXCEPTION", FakeLangDetectException), \
                patch("language_detector._langdetect") as mock_langdetect:
            mock_langdetect.detect_langs.side_effect = FakeLangDetectException("boom")
            lang, conf = detector.detect_with_confidence(
                "This is an English test sentence for fallback behavior",
            )
