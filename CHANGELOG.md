...
- **Phase II execution program** — [docs/ROADMAP_EXECUTION.md](docs/ROADMAP_EXECUTION.md) steps 9–17. **Already on main:** rungs 9–12 and the rung 14 apparatus. **On this branch, not previously on main:** the H5 living-bank verdict (fail-closed negative; do not promote). Rung 13 post-bank prune/re-anneal is the partial apparatus on main; detector-to-DAG prune is not on main. **Not done:** rungs 13, 15, 16, and 17. The ROADMAP Status column is the source of truth.
...
- **Lattice rungs 10–14 apparatus (already on main, git-verified):** SHIM/promotable-route substrate #284/#285/#286 (+ DoD correction #289, diagnostics observation-only); annealing controller #260 + post-bank temperature schedule #280; Evidence DAG schema #277; disintegration as post-bank prune/re-anneal #279 (+ builder/runtime/conditions #281/#282/#283); concept-drift harness #258–#266 (+ track-0 hygiene #267) with H3 teacher-supervised #287/#288, H4 `compound_cycles` knob #291, H5 head-to-head driver #290. Rungs **9–12 DONE**, **13 PARTIAL** (post-bank only), **14 apparatus DONE**. The H5 living-bank verdict is on this branch, not previously on main. This bullet does not mark historical SHIM-CD rows CLOSED.
- **Not done, and not on main:** rung 13 detector-to-DAG prune (open PR #293); rung 15 GNN prototype (no merged PR); rung 16 quant-aware routing (open PR #295); rung 17 disk pool (open PR #294). Do not call rungs 13, 15, 16, or 17 done.
