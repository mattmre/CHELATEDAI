...
- **Phase II execution program** — [docs/ROADMAP_EXECUTION.md](docs/ROADMAP_EXECUTION.md) steps 9–17. **Delivered apparatus:** rungs 9–12 (Model-Scope close-out, SHIM DoD, annealing controller, evidence DAG); 14 drift-campaign apparatus, with its H2/H4/H5 quantitative outputs now `LEGACY_METRIC_LINEAGE_BLOCKED`; 13 partial (post-bank prune/re-anneal lifecycle only). **Open:** 15 GNN prototype, 16 quant-aware shim routing plane, 17 disk pool slice. The ROADMAP Status column is the source of truth.
...
- **H5 legacy campaign lineage (quarantined, 2026-07):** the post-bank head-to-head apparatus and historical outputs are retained, but their exact nDCG values, comparator ordering, and stored gate outcome are `LEGACY_METRIC_LINEAGE_BLOCKED`. The metric helper formed IDCG from retrieved relevance rather than all positive qrels. The living-bank line remains conservatively **non-promoted** because no valid positive evidence exists; this is not a confirmed negative. Sources: [docs/drift-recovery-post-bank-headtohead-results-2026-06.md](docs/drift-recovery-post-bank-headtohead-results-2026-06.md), [docs/drift-recovery-post-bank-headtohead-nfcorpus-results-2026-06.md](docs/drift-recovery-post-bank-headtohead-nfcorpus-results-2026-06.md), and the complete immutable [v2 quarantine index](artifacts/legacy-ndcg-quarantine-index-v2.json).
- **H4 legacy compound-cycles ablation (quarantined):** the single-seed artifacts are retained, but their exact nDCG values and ordering are `LEGACY_METRIC_LINEAGE_BLOCKED`. They do not currently prove that compounding collapses, that the design is rejected, or that the idempotent fixed point is correct. Compounding remains non-promoted pending corrected regeneration. Source: [docs/drift-recovery-h4-compound-cycles-ablation-2026-07.md](docs/drift-recovery-h4-compound-cycles-ablation-2026-07.md).
- **H2 swap lineage (procedural only):** post-H1 campaign manifests and reports were recorded, and a preserved Git log blob independently proves that the real `all-mpnet-base-v2` backend resolved during the NFCorpus campaign. CD-A2-01 is closed only for that backend-resolution fact; CD-H1-01 is closed only for procedural supersession of the pre-H1 report lineage. All exact nDCG values, equality claims, effect sizes, and comparator orderings remain `LEGACY_METRIC_LINEAGE_BLOCKED` under CD-MLR-01. Sources: [docs/drift-recovery-swap-results-2026-06.md](docs/drift-recovery-swap-results-2026-06.md), [docs/drift-recovery-swap-nfcorpus-results-2026-06.md](docs/drift-recovery-swap-nfcorpus-results-2026-06.md), and the complete immutable [v2 quarantine index](artifacts/legacy-ndcg-quarantine-index-v2.json), which covers all 113 affected tracked artifacts while preserving the incomplete v1 sidecar for audit.
