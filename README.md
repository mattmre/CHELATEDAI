...
| Sweep / packaging / docs truth | JSON rewrite bounded; packaging still open | Each `run_large_sweep.py` result is one JSONL append. The JSON array is written once at the end. `run_large_sweep` is still absent from `pyproject.toml` `py-modules` (line 61). CI torch-cache documentation is not in this change. |
