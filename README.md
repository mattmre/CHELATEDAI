...
| Sweep / packaging / docs truth | JSON rewrite bounded; packaging still open | Each `run_large_sweep.py` result is one JSONL append. The JSON array is written once at the end. `run_large_sweep` and `sweep_result_store` are in `pyproject.toml` `py-modules`. A `--no-deps` install imports `sweep_result_store`. Importing `run_large_sweep` stops at missing `numpy`. CI torch-cache documentation is not in this change. |
