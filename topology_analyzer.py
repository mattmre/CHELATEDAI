...
class TopologyAnalyzer:
    ...
    @staticmethod
    def _empty_bond_result():
        """Return the empty bond-matrix result structure."""
        return {
    ...
    def _summarize_similarity_matrix(self, sim_matrix):
        """Build bond statistics from a precomputed similarity matrix."""
        sim_matrix = np.array(sim_matrix, dtype=float)
        n = sim_matrix.shape[0]

        if n == 0:
            return self._empty_bond_result()
        ...
        total_pairs = int(np.sum(off_diagonal_mask)) or 1

        bond_counts = {}
        bond_ratios = {}
        for bond_name, bond_code in BOND_TYPE_MAP.items():
            count = int(np.sum((bond_matrix == bond_code) & off_diagonal_mask))
    ...
    def build_bond_matrix(self, embeddings=None, similarity_matrix=None):
        """
        Build typed adjacency matrix from embeddings.

        Args:
            embeddings: numpy array of shape (n, dim)
            similarity_matrix: optional precomputed cosine similarity matrix
        ...
        if similarity_matrix is not None:
            sim_matrix = np.array(similarity_matrix, dtype=float)
            ...
            return self._summarize_similarity_matrix(sim_matrix)
    ...
    def compute_cluster_connectivity(self, embeddings, labels):
        ...
        sim_matrix = self._cosine_similarity_matrix(embeddings)
        bond_result = self.build_bond_matrix(similarity_matrix=sim_matrix)
        ...
        intra_mask = pair_mask & same_cluster_mask
        inter_mask = pair_mask & ~same_cluster_mask

        intra_codes = bond_matrix[intra_mask]
        inter_codes = bond_matrix[inter_mask]
        intra_counts = {
            bond_name: int(np.sum(intra_codes == bond_code))
            for bond_name, bond_code in BOND_TYPE_MAP.items()
        }
        inter_counts = {
            bond_name: int(np.sum(inter_codes == bond_code))
            for bond_name, bond_code in BOND_TYPE_MAP.items()
        }
        total_intra_pairs = int(np.sum(intra_mask))
        total_inter_pairs = int(np.sum(inter_mask))
        ...
        for label in unique_labels:
            ...
            if cluster_size > 1:
                ...
                cluster_cohesion[label_key] = float(np.mean(cohesion_vals))
            else:
                cluster_cohesion[label_key] = 1.0
