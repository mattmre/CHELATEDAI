...
def _build_default_datasets() -> Dict[str, DatasetInfo]:
    """Build a fresh copy of the default registry dataset metadata."""
    return {
        "SciFact": DatasetInfo(
            name="SciFact",
...
class BEIRDatasetRegistry:
    ...
    _DATASETS: Dict[str, DatasetInfo] = _build_default_datasets()
    ...
    def _reset_registry(cls):
        ...
        cls._DATASETS = _build_default_datasets()
...
class DatasetLoader:
    ...
    def _sample_corpus(
        ...
        corpus_keys_map = {str(key): key for key in corpus.keys()}
        all_ids = set(corpus_keys_map)
        required_ids = qrel_doc_ids & all_ids
        ...
        sampled = {
            corpus_keys_map[key]: corpus[corpus_keys_map[key]]
            for key in keep_ids
        }
