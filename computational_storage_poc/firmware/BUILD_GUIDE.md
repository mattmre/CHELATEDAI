...
The current firmware build validates the USB interception path with a deterministic, computed trigger-sector payload:

- sector `100` is intercepted in `tud_msc_read10_cb`,
- the firmware evaluates a small `4 -> 3 -> 2` graph with ReLU in C, and
- the result is returned as JSON for the host verifier.
