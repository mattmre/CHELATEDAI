PHASE B1 — BUILD D1 ONLY (paper statistics + fixes). You have write+execute access (sandbox bypassed).
Working dir = the agent-build worktree (this cwd), which contains the merged harness. Cached models: MiniLM-L6,
mpnet-base. Use HF_HUB_OFFLINE=1 HF_DATASETS_OFFLINE=1.

BUILD (write real code AND run it):
1. Create package research/drift_recovery/ per the locked plan below: harness_bridge.py (the ONLY module that
   imports the merged harness; include assert_harness_parity() proving packs reproduce the merged ndcg to <=1e-12),
   contracts.py, artifacts.py (EmbeddingPack), stats/paired_bootstrap.py, stats/learning_curve.py,
   stats/multiple_testing.py, d1/paper_stats.py.
2. Freeze the SciFact eval-split EmbeddingPack (Do, Dor, Qd, qrels, fit_idx, per-query scores) reproducing the
   canonical ladder (ridge ~84.4%, C3a ~20%) within 1e-3. The existing script scripts/ladder_evalsplit.py in the
   git-excluded waypoint folder shows the exact eval-split recipe (anchor_fraction=0.4, binary ndcg) — reuse its logic.
3. Run the 10k paired-query bootstrap -> 95% CIs on NDCG/recovery/dNDCG for ridge, Procrustes, MLP, C3a. FIX the
   C3a-as-scalar problem: score C3a per-query through the SAME ndcg (not the 0.1609 scalar). Report ridge-MLP and
   ridge-C3a contrast CIs with Holm.
4. Run the anchor-count learning curve {8,16,32,64,128} x 5 anchor-order seeds, same eval IDs; keep C3a (query-InfoNCE)
   and ridge (doc-pairs) as SEPARATE supervision types (do NOT share a fake n-anchors axis).
5. Emit machine-readable JSON tables + a short markdown report of the CIs to research/drift_recovery/out/d1/.

ALSO PRODUCE (do NOT edit the paper; it is git-excluded — output for the chair to apply):
6. The full Section 3 (System Under Test) draft: adapter forward maps, the EXACT bounded-adapter formula + its floor,
   and the controller drift->temperature->should_correct schedule. Read the real harness (chelation_adapter.py,
   config.py, and the annealing controller) to get true definitions. Write to research/drift_recovery/out/d1/section3_draft.md.
7. A precise edit-list (research/drift_recovery/out/d1/paper_edits.md): every paper location that says 'ceiling' that
   must become 'observed plateau under this protocol', and any number to update from the new CIs.

If execution is blocked by the environment, still WRITE all code + a single run_d1.py and say so explicitly.
Do NOT build D2 or D3 yet. Below is the LOCKED plan for context.
