...
| CD-A2-01 | The real swap-backend resolution path (`query_encoder_drift.QueryEncoderDrift._backend` → `embedding_backend.create_embedding_backend(swap_model_name)`, loading the real `all-mpnet-base-v2`) is no longer exercised by any default-CI test after the real-model smoke was made opt-in (gated on `CHELATED_RUN_REAL_MODEL_TESTS=1`) to stop a ~17-min HF-connection CI hang. It is covered only by mocked unit tests and the non-gating PR-A4 real-model campaign. L5 (untested production path in default CI). | PR for real-model-smoke opt-in (2026-06-14) | expired | NO — cleared from committed manifests on this branch; not a live block | **CLOSED** — this close rests on the committed campaign manifests, and the GPU campaign was not re-executed in this session. Both `experiment_runs/drift-recovery/swap/swap-campaign-manifest-2026-06.json` and `experiment_runs/drift-recovery/swap-nfcorpus/swap-campaign-manifest-2026-06.json` have `config.swap_model` `all-mpnet-base-v2` and `arena` `query_encoder_swap`. Commit `2b0fa04` deleted the NFCorpus per-condition run JSONs and did not commit replacements. Opened `experiment_runs/drift-recovery/h4-compound/C4a_compound0_seed42.json` (same `query_encoder_swap` driver; committed on this branch) and pre-deletion `experiment_runs/drift-recovery/swap-nfcorpus/main/C3a_seed42.json` (origin/main, removed by `2b0fa04`; its `baseline_ndcg` is still the contaminated `0.5563402674063337`): both record `drift_manifest.swap_model` `all-mpnet-base-v2`, `swap_dim` 768, `store_dim` 384, and `config.device` `cuda`. They do not name a Python backend class and do not contain `mock`. `swap_dim` is the measured `embed_raw` width; the campaign driver does not inject a stub (in-repo stub dim 96; real `all-mpnet-base-v2` is 768). The result docs only echo the configured model string. `TestQueryEncoderDriftRealModelSmoke` was not re-run in this session. |
...
| CD-H1-01 | The committed swap-campaign result docs (`docs/drift-recovery-swap-results-2026-06.md`, `docs/drift-recovery-swap-nfcorpus-results-2026-06.md`) carry C3a baseline AND final NDCG (+ the C3a budget sweep) generated before the H1 fix (commit `5b2379b`), which removed a build-time bounded adapter that contaminated C3a's ingested vectors / pre-correction baseline. C3a's ingest, baseline, and trained-adapter snapshot all shift after the fix; C0/C2/C2O/C4a are unaffected. The stale C3a numbers are disclosed inline in both docs. L13 (stale-docs). | H1 (PR pending) | expired | NO — cleared from committed manifests on this branch; not a live block | **CLOSED** — read from the committed JSON, not the PR body; the GPU campaign was not re-executed in this session. SciFact seed 42 `baseline_ndcg` for C0/C2/C2O/C3a/C4a is `0.7934593935363062` in `experiment_runs/drift-recovery/swap/swap-campaign-manifest-2026-06.json`. NFCorpus seed 42 for those five conditions is `0.5550044219616403` in `experiment_runs/drift-recovery/swap-nfcorpus/swap-campaign-manifest-2026-06.json` (seeds 1337 and 7 also match across conditions: `0.5936171482249235` and `0.6443047069474342`). On origin/main before this branch, NFCorpus seed-42 C3a baseline was `0.5563402674063337` and did not match; SciFact seed-42 baselines already matched. |
