...
**SUPERSEDED METRIC EVIDENCE — STILL NON-PROMOTED.** A later metric-lineage
audit found that the legacy nDCG helper forms IDCG from retrieved relevance
instead of all positive qrels. H5's exact values and comparator ordering are
therefore `LEGACY_METRIC_LINEAGE_BLOCKED` and are not accepted scientific
evidence until corrected regeneration. The existing non-promotion remains the
conservative disposition because the line has no valid positive evidence; this
does not reconfirm the stored comparator ordering.

The living-bank / annealed-post-bank corrector line was parked per its own
preregistered H5 gate: C5 (living) had to beat **both** C5s (frozen static bank)
and C5r (one-shot router). These are quarantined stored campaign values, not
current accepted metrics (query-encoder-swap arena, cycles 12, seeds
[42,1337,7]):
...
exact nDCG values are quarantined by the same lineage audit; compounding
remains non-promoted pending corrected regeneration.
...
**Last session**: 2026-07-27 — PR #292 Tier-B reconditioning added first-cycle CD-MLR-01 and DS-MLR-01, expanded the legacy nDCG quarantine to an exact 113/113 tracked-artifact v2 inventory with original-versus-warning prose provenance, added hostile fail-closed validation plus an explicit CI gate, and retained only a narrow durable backend-resolution proof. The flag remains CLEAR only because CD-MLR-01 has not yet survived a full cycle; it becomes BLOCKED if the debt expires open. Prior: 2026-06-13 PR #267 Track 0 hygiene closed CD-247-01 / CD-247-02.
