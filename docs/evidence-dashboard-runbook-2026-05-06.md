...
The workflow's `cleanup-guard-mode` input defaults to `fail`; use `warn` only for deliberate nonblocking evidence collection. Warn mode still prints cleanup-review allowed/blocked status and source status.
